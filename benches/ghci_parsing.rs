...
fn create_sample_ghc_outputs() -> Vec<(&'static str, String)> {
    vec![
        (
            "simple_error",
            include_str!("./fixtures/simple_error.txt").to_string(),
        ),
        (
            "multiple_errors",
            include_str!("./fixtures/multiple_errors.txt").to_string(),
        ),
        (
            "warnings",
            include_str!("./fixtures/warnings.txt").to_string(),
        ),
        (
            "mixed_output",
            include_str!("./fixtures/mixed_output.txt").to_string(),
        ),
        ("large_output", {
            let base_error = r#"
...
"#;
        ...
        ("ansi_heavy", {
            let base =
                "\x1b[1;31msrc/Main.hs:1:1: \x1b[0m\x1b[1merror:\x1b[0m parse error on input\n";
            base.repeat(50)
        }),
    ]
}
...
fn generate_compilation_output(modules: usize) -> String {
    ...
    for i in 1..=modules {
        ...
        if i % 5 == 0 {
            output.push_str(&format!(
                "src/Module/Path/Component{}.hs:10:5: warning:\n",
                i
            ));
